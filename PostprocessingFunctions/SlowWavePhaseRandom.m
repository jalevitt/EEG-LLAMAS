function [vars, Graph, EEG] = SlowWavePhaseRandom(EEG, vars, Graph)
    ...
    if (vars.currentPosition - vars.TriggerBuffer) > vars.LastStimPosition
        if Mag > EEG.Threshold
            if (PredAngle < - 9 * pi/10 || PredAngle > 9 * pi/10)
                RandomDelay = rand * 2;
                PsychPortAudio('Start', vars.audio_port, vars.repetitions, vars.ChunkTime + vars.SlowWaveDelay + RandomDelay, 0);
                %sound(Sound, fsSound)
                vars.StimTimes(vars.StimCount) = round(vars.currentPosition + vars.SlowWaveDelay * EEG.fs);
                vars.StimCount = vars.StimCount + 1;
                vars.LastStimPosition = vars.currentPosition + round(RandomDelay / EEG.fs);
            end
        end
    end
end
