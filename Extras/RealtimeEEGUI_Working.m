function varargout = RealtimeEEGUI_Working(varargin)
% RealtimeEEGUI_Working MATLAB code for RealtimeEEGUI_Working.fig
%      RealtimeEEGUI_Working, by itself, creates a new RealtimeEEGUI_Working or raises the existing
%      singleton*.
%
%      H = RealtimeEEGUI_Working returns the handle to a new RealtimeEEGUI_Working or the handle to
%      the existing singleton*.
%
%      RealtimeEEGUI_Working('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in RealtimeEEGUI_Working.M with the given input arguments.
%
%      RealtimeEEGUI_Working('Property','Value',...) creates a new RealtimeEEGUI_Working or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before RealtimeEEGUI_Working_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to RealtimeEEGUI_Working_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help RealtimeEEGUI_Working
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @RealtimeEEGUI_Working_OpeningFcn, ...
                   'gui_OutputFcn',  @RealtimeEEGUI_Working_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before RealtimeEEGUI_Working is made visible.
function RealtimeEEGUI_Working_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to RealtimeEEGUI_Working (see VARARGIN)

% Choose default command line output for RealtimeEEGUI_Working
handles.output = hObject;
handles.Recording = [];
% Update handles structure
guidata(hObject, handles);

% UIWAIT makes RealtimeEEGUI_Working wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = RealtimeEEGUI_Working_OutputFcn(hObject, eventdata, handles) 
...
% Clear our workspace
    ...
    vars.audio_port = PsychPortAudio('Open', 3, 1, [], fsSound, 2, [], 0.02);
